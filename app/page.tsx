...
import { DEFAULT_AUTHFORM_STATE } from "./_data/AuthFormPresets";
...
import { SECTIONS, type SectionId, type AuthFormState, type StudioPreset } from "./types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<AuthFormState>(DEFAULT_AUTHFORM_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const [activePresetId, setActivePresetId] = useState<string | null>(null);
  const [downloadName] = useState("auth-form-component");
  ...
  const update = <K extends keyof AuthFormState>(key: K, value: AuthFormState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_AUTHFORM_STATE, ...(preset.state as Partial<AuthFormState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Auth Form Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
