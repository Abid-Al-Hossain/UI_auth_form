...
export function buildReactCode(state: AuthFormState) {
  return [
    ...
    "  return { width: state.width, minHeight: state.height, padding: state.padding, display: \"grid\", gap: state.gap, borderRadius: state.radius, border: state.borderWidth + \"px solid \" + state.border, boxShadow: \"0 \" + Math.round(state.shadow / 3) + \"px \" + state.shadow + \"px rgba(0,0,0,.28)\", background: state.background, color: state.foreground, fontFamily: state.fontFamily, opacity: state.disabled ? 0.55 : 1 };",
    "}",
    "",
